...
class Qwen3VLModel(MegatronModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # TE flash-attention consumes the *global* packed cu_seqlens and rebuilds
        # its per-CP-rank boundaries internally, but GDN/SSM layers validate
        # cu_seqlens[-1] against the CP-sharded local length. Hand those layers a
        # cu_seqlens rebased into this rank's local coordinate system.
        gdn_packed_seq_params = (
            get_cp_local_packed_seq_params(packed_seq_params, packed_cp_index)
            if packed_seq_params is not None and packed_cp_index is not None
            else None
        )
